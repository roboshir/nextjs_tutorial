...
- 自分で考えた指数名を付けて公開する
- ほかのユーザーが作成した指数を見る
- 公開指数を選び、指数値・チャート・構成銘柄の詳細を見る

ログイン、管理画面、外部株価API、決済、ユーザーによる株価入力はありません。公開指数の共有保存に限り、Neon Postgresを使用します。

## Vercelで共有保存を有効にする

1. Vercelで対象プロジェクトを開く
2. **Storage** または **Marketplace** から **Neon** を追加する
3. 新しいPostgresデータベースを作り、このVercelプロジェクトへ接続する
4. `DATABASE_URL` が環境変数へ追加されたことを確認する
5. GitHubへ変更をpushし、Vercelで再デプロイする

最初のアクセス時に必要なテーブルが自動作成されます。接続文字列をコードやGitHubへ直接書かないでください。
